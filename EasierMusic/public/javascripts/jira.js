...
(function () {
    ...
    this.getDataManagerStoppingReleaseIssues = function (callback) {
        var jql =
            "project = DBMGR AND issuetype = Bug AND status in (Reported, \"Ready for Assignment\") AND priority in (\"Very High\", \"Stopping Release\") ORDER BY created ASC";

        this.searchJira(jql, null, callback);
    };

    this.getEstimatorStoppingReleaseIssues = function (callback) {
        var jql =
            "project = ESTIMATOR AND issuetype = Bug AND status in (Reported, \"Ready for Assignment\") AND priority in (\"Very High\", \"Stopping Release\") ORDER BY created ASC";

        this.searchJira(jql, null, callback);
    };

    this.getUsersDevCompleteBugsEstimator = function (callback) {
        var jql =
            "project in (\"DESTINI Estimator\") AND issuetype = Bug AND status in (\"Development Completed\") AND updated >= -6d AND priority = \"Stopping Release\" AND status = \"Development Completed\" AND assignee = currentUser() ORDER BY priority DESC, updated DESC";

        this.searchJira(jql, null, callback);
    };

    this.getUsersDevCompleteBugsDataManager = function (callback) {
        var jql =
            "project in (\"DESTINI Database Manager\") AND issuetype = Bug AND status in (\"Development Completed\") AND updated >= -6d AND priority = \"Stopping Release\" AND status = \"Development Completed\" AND assignee = currentUser() ORDER BY priority DESC, updated DESC";

        this.searchJira(jql, null, callback);
    };

    //Gets all the editable fields of an issue
    this.editableFields = function (issueNum, callback) {

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/' + issueNum + '/editmeta'),
            method: 'GET'
        };
        ...
        this.doRequest(options, function (error, response) {
            ...
            callback(response.statusCode + ': Error getting editable fields)');
    ...
    this.getUsersIssues = function (username, open, callback) {
        ...
        var project = "DESTINI Database Manager";
